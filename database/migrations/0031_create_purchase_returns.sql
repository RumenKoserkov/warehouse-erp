CREATE TABLE purchase_returns (
    ...
    active_purchase_id BIGINT UNSIGNED
        GENERATED ALWAYS AS (
            CASE
                WHEN status = 'draft'
                    THEN purchase_id
    ...
    UNIQUE KEY unique_purchase_return_number (
        company_id,
        return_number
    ),

    UNIQUE KEY unique_open_return_per_purchase (
        company_id,
        active_purchase_id
    ),

    INDEX index_purchase_returns_company_date (
        company_id,
        return_date
    ),

    INDEX index_purchase_returns_purchase (
        company_id,
        purchase_id,
        status
    ),

    INDEX index_purchase_returns_warehouse (
        company_id,
        warehouse_id,
        status
    ),

    CONSTRAINT fk_purchase_returns_company
    ...
    CONSTRAINT fk_purchase_returns_completed_by
        FOREIGN KEY (completed_by_user_id)
        REFERENCES users(id)
        ON DELETE SET NULL,

    CONSTRAINT fk_purchase_returns_cancelled_by
...
CREATE TABLE purchase_return_items (
    ...
    purchased_quantity DECIMAL(14,3) NOT NULL,
    ...
    UNIQUE KEY unique_purchase_return_item (
        purchase_return_id,
        purchase_item_id
    ),

    INDEX index_purchase_return_items_company (
        company_id,
        purchase_return_id
    ),

    INDEX index_purchase_return_items_source (
        company_id,
        purchase_item_id
    ),

    INDEX index_purchase_return_items_product (
        company_id,
        product_id
    ),
    ...
    CONSTRAINT fk_purchase_return_items_company
        FOREIGN KEY (company_id)
        REFERENCES companies(id)
        ON DELETE CASCADE,

    CONSTRAINT fk_purchase_return_items_source
